// Системы вентилятор - заслонка с дублированием с общим нагревателем

import Damper, { DamperMode } from "../machines/Damper";
import FDH from "../subsystems/FanDamperHeater";

import cron from "node-cron";
import { FanMode } from "../machines/Fan_diff";
...
export default class FD2H_D_fire {
  ...
  constructor(
    ...
    this.bus.on("machineError", (payload) => {
      const msg = `${payload.subsystemName}. ${payload.message}`;
  ...
  public async start() {
    try {
      ...
      if (this.mode === SystemMode.HANDMODE && (this.stage === SystemStage.STARTING || this.stage === SystemStage.WORKING))
        return;
      if (this.stage === SystemStage.STOPING) {
        return;
      }
      ...
      if (isEven) {
        ...
        await this.subsystem1.start();
      } else {
        await this.inletDamper.open();
        this.bus.emit(`MQTTSend`, { value: "true", topic: `${this.inletDamper.subsystemName}` });
        await this.subsystem1.stopWithoutPurge();
        await this.subsystem2.start();
      }
  ...
  public async stop() {
    try {
      ...
      console.log("________STOP SYSTEM_________");
  ...
  public async auto() {
    try {
      ...
      if (isEven) {
        await this.inletDamper.open();
        await Promise.all([await this.subsystem1.stopWithoutPurge(), await this.subsystem2.start()]);
      } else {
        await this.inletDamper.open();
        await Promise.all([await this.subsystem2.stopWithoutPurge(), await this.subsystem1.start()]);
      }
