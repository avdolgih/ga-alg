...
export default class FD2H_D_fire_machine {
  ...
  public async start() {
    try {
      ...
      if (isEven) {
        ...
        if (this.stage === SystemStage.STARTING) this.heater.start();
  ...
  public async stop() {
    try {
      ...
      if (this.heater.mode === HeaterElectricMode.INPROGRESS || this.heater.mode === HeaterElectricMode.WORKING) {
        this.heater.stop();
  ...
  public fireStop = async (value: boolean) => {
    if (!value) {
      ...
      if (this.heater.mode === HeaterElectricMode.ERROR) return;
      this.resetError();
    }
  };
